# Java class的基本结构，Klass的相关数据结构及作用
...
![](.\picture\class_structure\12.png)
...
![](.\picture\class_structure\13.png)
...
- 实例数据区域(fields data)：实例字段没有在OopDesc类中定义相应属性存储，所以只能申请一定大小的空间，按顺序存储，对象字段存放紧跟着OopDesc实例本身占用的内存空间之后，获取时只能通过偏移来取值。
...
> Synthetic_attribute {
>   u1   attribute_name_index; //指向常量池的名称引用
>   u1   attribute_length; // 固定为1
> }
...
​	用于表示某个类、字段或者方法，已经被程序作者定为不在推荐使用，它可以通过代码中使用@deprecated 注解进行设置。
